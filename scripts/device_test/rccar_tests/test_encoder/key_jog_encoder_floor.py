...
DEFAULT_LOG_HZ = 20.0
DEFAULT_LOG_CSV = "key_jog_encoder_floor_log.csv"
...
def set_pwm(pi, esc_duty: float) -> None:
    ...
    steering_status = pi.hardware_PWM(
        STEERING_GPIO,
        PWM_HZ,
        duty_to_pigpio(STEERING_NEUTRAL_DUTY),
    )
...
def require_confirmations(args: argparse.Namespace) -> None:
    required = {
        "--confirm-floor-clear": args.confirm_floor_clear,
        ...
        "--confirm-steering-neutral": args.confirm_steering_neutral,
...
def run_hardware(args: argparse.Namespace) -> None:
    ...
    print_configuration(args)
    print("[FLOOR] 平坦で十分に広い直線路だけで使用してください")
    print("[RISK] 無負荷時10.100 %で約1.7〜1.8 m/sでした。床上速度は未確認です")
    ...
    try:
        ...
        print("中立を出力しました。車両前方を再確認後、r、wの順で操作してください")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="床上の直進Jogとエンコーダ速度計測を単体で実行する"
    )
    parser.add_argument("--enable-hardware", action="store_true")
    parser.add_argument("--confirm-floor-clear", action="store_true")
    ...
    parser.add_argument("--confirm-steering-neutral", action="store_true")
    ...
    if not 0.0 < args.run_seconds <= MAX_RUN_SECONDS:
        parser.error(f"--run-secondsは0より大きく{MAX_RUN_SECONDS:.1f}以下[s]にしてください")
    if args.log_hz <= 0.0:
